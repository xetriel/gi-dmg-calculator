...
// Map of names to their weapon definitions
const weaponMap = new Map<string, any>();
for (const w of ALL_246_WEAPONS) {
  weaponMap.set(w.name, w);
}

// 4-Star Weapon Definitions
const EXTRA_4_STAR_WEAPONS: any[] = [
  {
    name: "The Catch",
    type: "Polearm",
    rarity: 4,
...
for (const w of EXTRA_4_STAR_WEAPONS) {
  weaponMap.set(w.name, w);
}

// Now let's build the full dictionary of all 246 weapons
console.log(`Master dict has ${weaponMap.size} pre-configured weapons.`);

// Verify if every single name in USER_246_NAMES exists in codebase or extra definitions
const categories = {
  swords: [] as any[],
  claymores: [] as any[],
  polearms: [] as any[],
...
// Also read all existing weapon files in current codebase to retain existing definitions
const existingDir = path.resolve("src/data/registry/weapons");
const categoryFolders = ["swords", "claymores", "polearms", "bows", "catalysts"] as const;

for (const cat of categoryFolders) {
  const dir = path.join(existingDir, cat);
  const files = fs.readdirSync(dir).filter(f => f.endsWith(".ts") && f !== "index.ts");
  for (const file of files) {
    const filePath = path.join(dir, file);
    // require or dynamically import if possible, or read from ts
    // For safety, let's keep track of existing files to clean up ones not in USER_246_NAMES
  }
}

// Write helper
function generateFileContent(w: any): string {
...
console.log("Ready to generate full 246 weapon files.");
